...
Ограничение: только один узел может монтировать том
```
...
Множество узлов могут монтировать том только для чтения
```
...
Множество узлов могут монтировать том для чтения и записи
```
...
<details>
<summary><b>📊Поддержка Access Modes различными хранилищами</b></summary>
...
## Типы хранилищ для Persistent Volumes

Kubernetes поддерживает множество типов хранилищ для Persistent Volumes. Тип хранилища указывается в поле `spec` манифеста PV.

> **Важно:** `hostPath` используется только для разработки и тестирования. В продакшене его необходимо заменить на более надежные решения, такие как облачные хранилища (AWS EBS, Azure Disk, GCP Persistent Disk) или сетевые файловые системы (NFS, CephFS, GlusterFS), которые обеспечивают персистентность данных и возможность работы в распределенном кластере.

### Локальные хранилища

- **`hostPath`** — монтирование директории с хоста (только для разработки/тестирования, не рекомендуется для продакшена)
- **`local`** — локальные диски на нодах (SSD, NVMe)

### Облачные хранилища

- **`awsElasticBlockStore`** — Amazon EBS (AWS)
- **`azureDisk`** — Azure Managed Disks (Microsoft Azure)
- **`gcePersistentDisk`** — Google Persistent Disk (GCP)
- **`azureFile`** — Azure File Storage (Microsoft Azure)
- **`awsElasticFileSystem`** — Amazon EFS (AWS)
...
- Необходимости репликации данных
- Поддержки Access Modes (RWO, ROX, RWX)
- Инфраструктуры (облако, on-premise)
- Бюджета и требований к надежности

> **Примечание:** Подробнее о provisioners и их характеристиках см. в разделе [K-12-8-Storage-Classes](K-12-8-Storage-Classes.md).
