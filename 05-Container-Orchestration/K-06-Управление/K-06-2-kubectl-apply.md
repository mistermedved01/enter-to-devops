...
✅ **kubectl apply** — декларативная команда для управления объектами Kubernetes

✅ **Три конфигурации:**
- Local (локальный файл) — что мы хотим
- Live (живая конфигурация) — что есть в кластере
- Last-applied (последняя примененная) — что было применено ранее

✅ **Механизм работы:**
- Last-applied хранится в аннотациях объекта
- Apply использует 3-way merge (local, last-applied, live)
- Позволяет безопасно удалять поля из конфигурации
