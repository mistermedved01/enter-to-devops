...
✅ **Императивный подход** — команды напрямую создают/изменяют объекты

✅ **Декларативный подход** — описание желаемого состояния в YAML файлах

✅ **Сравнение:**
- Императивный: быстро, просто, но нет версионности
- Декларативный: медленнее, но версионность, Git-контроль, рекомендуется для production

✅ **Best Practices:**
- Для продакшена: всегда используйте декларативный подход
- Для экзамена: императивный для простых задач, декларативный для сложных

✅ **Ключевые команды:**
- Императивные: `kubectl run`, `create`, `expose`, `scale`, `set image`
- Декларативные: `kubectl apply`, `diff`, `delete -f`
